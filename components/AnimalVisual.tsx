...
export default function AnimalVisual({ habitId, habitName, className }: AnimalVisualProps) {
  ...
  const loadAnimal = async () => {
    try {
      ...
      // Sync first to ensure data is correct, then fetch (no delays needed)
      await syncAnimalProgressForHabit(habitId).catch(err => {
        console.error('Error syncing animal:', err)
      })
      
      // Fetch animal data (syncFirst=true ensures it's synced)
      const data = await getCurrentAnimal(habitId, true)
      setAnimalData(data)
    } catch (error) {
